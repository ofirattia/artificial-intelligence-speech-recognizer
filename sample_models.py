...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    rnn_start = GRU(units, activation='relu',
        return_sequences=True, implementation=2, name='rnn_start')(input_data)
    
    next_rnn_input_as_rnn = rnn_start
    for layer in range(recur_layers - 1):
        layer_name = "rnn_" +  str(layer)
        rnn = GRU(units, activation="relu",return_sequences=True, 
                 implementation=2, name=layer_name) (next_rnn_input_as_rnn)
        
        rnn_output = BatchNormalization(name="bn_" + str(layer))(rnn)
        next_rnn_input_as_rnn = rnn_output
        
        
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(next_rnn_input_as_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    # Add convolutional layer
    one_dim_con_layer = Conv1D(filters, kernel_size, 
                     strides=conv_stride, 
                     padding=conv_border_mode,
                     activation='relu',
                     name='conv1d')(input_data)
    ...
    # Add batch normalization
    bn_cnn = BatchNormalization(name='bn_conv_1d')(one_dim_con_layer)
